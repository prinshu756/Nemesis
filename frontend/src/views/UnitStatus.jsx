  ...
  return (
    <div className="view-unit-status" id="view-unit-status">
      ...
      <div style={{ marginTop: 12 }}>
        <h2 className="font-heading" style={{ fontSize: 14, fontWeight: 700, marginBottom: 12, letterSpacing: '1px' }}>
          📱 NETWORK_MONITORING
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          <div className="card">
            <div className="card-body" style={{ textAlign: 'center', padding: '12px' }}>
              <div className="font-mono text-dim" style={{ fontSize: 9, marginBottom: 4 }}>Total Devices</div>
              <div className="font-heading text-cyan" style={{ fontSize: 24, fontWeight: 700 }}>
                {state.backendStatus?.network_monitoring?.total_devices || totalDeployed}
              </div>
            </div>
          </div>
          <div className="card">
            <div className="card-body" style={{ textAlign: 'center', padding: '12px' }}>
              <div className="font-mono text-dim" style={{ fontSize: 9, marginBottom: 4 }}>Active Devices</div>
              <div className="font-heading text-green" style={{ fontSize: 24, fontWeight: 700 }}>
                {state.backendStatus?.network_monitoring?.active_devices || operational}
              </div>
            </div>
          </div>
          <div className="card">
            <div className="card-body" style={{ textAlign: 'center', padding: '12px' }}>
              <div className="font-mono text-dim" style={{ fontSize: 9, marginBottom: 4 }}>High Risk</div>
              <div className="font-heading text-red" style={{ fontSize: 24, fontWeight: 700 }}>
                {state.backendStatus?.network_monitoring?.high_risk_devices || 0}
              </div>
            </div>
          </div>
          <div className="card">
            <div className="card-body" style={{ textAlign: 'center', padding: '12px' }}>
              <div className="font-mono text-dim" style={{ fontSize: 9, marginBottom: 4 }}>Isolated</div>
              <div className="font-heading text-amber" style={{ fontSize: 24, fontWeight: 700 }}>
                {state.backendStatus?.network_monitoring?.devices_isolated || 0}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Active Assets - Devices with Hostname */}
      <div style={{ marginTop: 12 }}>
        <h2 className="font-heading" style={{ fontSize: 14, fontWeight: 700, marginBottom: 12, letterSpacing: '1px' }}>
          ◻ ACTIVE_ASSETS_DETAIL
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 8 }}>
          {sorted.slice(0, 12).map(dev => (
            <div key={dev.mac} className="card" style={{ padding: 12 }}>
              <div className="font-mono" style={{ fontSize: 10, marginBottom: 4 }}>
                <div className="text-cyan" style={{ fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {dev.hostname || 'Device'}
                </div>
                <div className="text-dim" style={{ fontSize: 9, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {dev.ip || 'N/A'}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 4, alignItems: 'center', marginBottom: 6 }}>
                <span className={`badge ${
                  ...
                  dev.risk_level === 'high' ? 'badge-warning' :
                  'badge-online'
                }`} style={{ fontSize: 8 }}>
                  {dev.status === 'isolated' ? 'ISO' :
                   dev.risk_level === 'critical' ? 'CRT' :
                   dev.risk_level === 'high' ? 'HI' :
                   'OK'}
                </span>
                <span className="font-mono text-dim" style={{ fontSize: 8 }}>
                  {dev.device_type || 'Unknown'}
                </span>
              </div>
              <div className="progress-bar" style={{ marginBottom: 4, height: 3 }}>
                <div className={`progress-fill ${dev.risk_level === 'critical' ? 'red' : dev.risk_level === 'high' ? 'amber' : 'cyan'}`}
                  style={{ width: `${(dev.risk_score || 0) * 10}%`, height: '100%' }} />
              </div>
              <div className="font-mono text-dim" style={{ fontSize: 8 }}>
                Risk: {(dev.risk_score || 0).toFixed(1)}
              </div>
            </div>
          ))}
        </div>
